...
#include "traffic_sign_model_data_MNV1.h"
...
printf("Klasse = %d | Wahrscheinlichkeit = %d\n\n", max_class_conf_index, max_class_conf);

// Ab hier beginnt die Validation
const int TP_Class = 5;
int8_t class_conf_theshold = 100;

// Gesuchte Klasse:| Geradeaus = 0, Kreisverkehr = 1, Links = 2, negativ = 3, Rechts = 4, Stop = 5
if (max_class_conf >= class_conf_theshold)
{    
    // Zähler True Positiv | TP
    if (max_class_conf_index == TP_Class)
    {
      TP = TP + 1;
    }

    // Zähler False Positiv | FP
    if (max_class_conf_index != TP_Class)
    {
      FP += 1;
    }
    ...
    // Gesuchte Klasse:| Geradeaus = 0, Kreisverkehr = 1, Links = 2, negativ = 3, Rechts = 4, Stop = 5
    if (max_class_conf_index == 0)
    {
      Geradeaus += 1;
    }

    if (max_class_conf_index == 1)
    {
      Kreisverkehr += 1;
    }

    if (max_class_conf_index == 2)
    {
      Links += 1;
    }

    if (max_class_conf_index == 3)
    {
      Negativ += 1;
    }

    if (max_class_conf_index == 4)
    {
      Rechts += 1;
    }

    if (max_class_conf_index == 5)
    {
      Stop += 1;
    }
}
...
vorhersagen_gesamt =  vorhersagen_gesamt + 1; 

printf("Geradeaus = %d | Kreisverkehr = %d | Links = %d | Negativ = %d | Rechts = %d | Stop = %d\n\n", Geradeaus, Kreisverkehr, Links, Negativ, Rechts, Stop);
printf("TP = %.1f | FP = %.1f | FN = %.1f\n\n", TP, FP, FN); 
printf("Vorhersagen = %d | Recall = %.2f | Precision = %.2f | ",vorhersagen_gesamt, recall, precision);
