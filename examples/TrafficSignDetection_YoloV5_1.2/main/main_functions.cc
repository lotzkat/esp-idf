...
void loop() {
  ...
  printf("Dimension Inputtensor = %d\n", input->dims->size);
  ...
  int index = 0;
    ...
    for(int i = 4; i <= num_output; i = i + 10)
    {
      ...
      float class_4_score = output->data.f[i + 5];
        ...
        if(objectness_score > conf0_max )
        {
          conf0_max = objectness_score;
          index = i;
        }
  ...
  float end = clock();
  int time = end - start;
  int PP_Time = (end - PP_Start) * 1000;
  printf("Zeit pro Durchlauf = %d Zeit für Postprocessing = %d ms\n", time, PP_Time);
